...
extension TotalViewController: UICollectionViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        delgate?.bb(cc: scrollView.contentOffset.y)
//        if scrollView.contentOffset.y >= 40 {
//            self.navigationController?.navigationBar.isHidden = true
////            aaa.alpha = 0.5*(scrollView.contentOffset.y)/360+(4/9)
//        } else {
...
extension TotalViewController {
    ...
    private func createMustWatchSection() -> NSCollectionLayoutSection {
        ...
        let header = NSCollectionLayoutBoundarySupplementaryItem(layoutSize: .init(widthDimension: .fractionalWidth(1.0), heightDimension: .absolute(620)), elementKind: UICollectionView.elementKindSectionHeader, alignment: .top)
    ...
    private func createFamousLiveChannelSection() -> NSCollectionLayoutSection {
        let item = NSCollectionLayoutItem(layoutSize: .init(widthDimension: .fractionalWidth(1), heightDimension: .fractionalHeight(1)))
        item.contentInsets = .init(top: 10, leading: 5, bottom: 10, trailing: 5)
        let group = NSCollectionLayoutGroup.horizontal(layoutSize: .init(widthDimension: .absolute(Constant.Size.width/2.5), heightDimension: .estimated(150)), subitems: [item])
        let section = NSCollectionLayoutSection(group: group)
        section.contentInsets = .init(top: 0, leading: 0, bottom: 20, trailing: 0)
        section.orthogonalScrollingBehavior = .continuous
        let header = NSCollectionLayoutBoundarySupplementaryItem(layoutSize: .init(widthDimension: .fractionalWidth(1.0), heightDimension: .absolute(20.0)), elementKind: UICollectionView.elementKindSectionHeader, alignment: .top)
